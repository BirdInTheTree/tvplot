...
class TestNetworkRetry:
    ...
    async def test_retries_on_transient_error(self, config):
        ...
        with patch("tvplot.llm._araw_call", side_effect=flaky_raw_call), \
             patch("tvplot.llm.asyncio.sleep", new_callable=AsyncMock):
    ...
    async def test_raises_after_max_retries(self, config):
        ...
        with patch("tvplot.llm._araw_call", side_effect=always_fail), \
             patch("tvplot.llm.asyncio.sleep", new_callable=AsyncMock):
    ...
    async def test_backoff_delays(self, config):
        ...
        with patch("tvplot.llm._araw_call", side_effect=fail_twice), \
             patch("tvplot.llm.asyncio.sleep", sleep_mock):
            await _araw_call_with_retry("sys", [{}], config, False)
...
class TestPartialParallel:
    ...
    async def test_preserves_successful_retries_failed(self, config):
        ...
        with patch("tvplot.llm.acall_llm", side_effect=mock_acall_llm):
...
class TestResumeValidation:
    ...
    def test_cast_without_plotlines_raises(self):
        ...
        from tvplot.models import CastMember
    ...
    def test_plotlines_without_cast_raises(self):
        from tvplot.pipeline import get_plotlines
        from tvplot.models import Plotline
    ...
    def test_breakdowns_length_mismatch_raises(self):
        from tvplot.pipeline import get_plotlines
        from tvplot.models import EpisodeBreakdown
...
class TestBatchTimeout:
    ...
    async def test_batch_polling_timeout(self, config):
        import tvplot.llm as llm_module
        ...
        try:
            ...
            with patch("anthropic.AsyncAnthropic", return_value=mock_client), \
                 patch("tvplot.llm.asyncio.sleep", new_callable=AsyncMock):
                from tvplot.llm import acall_llm_batch
